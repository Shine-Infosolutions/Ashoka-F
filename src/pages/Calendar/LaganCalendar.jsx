...
function LaganCalendar() {
    ...
    const fetchBookings = async () => {
      try {
        const res = await axios.get(
          "https://ashoka-b.vercel.app/api/bookings"
        );
        const grouped = {};
        res.data.forEach((b) => {
  ...
  const dateTemplate = ({ year, month, day }) => {
    ...
    return (
      <div
        className={`w-10 h-12 md:w-14 md:h-14 relative rounded-lg flex items-center justify-center cursor-pointer transition-all duration-150 ${highlightClass} hover:ring-2 hover:ring-[#c3ad6b]`}
      ...
      >
        <span className="text-base md:text-lg font-semibold text-black select-none">
          {day}
        </span>
        {hasBooking && (
          <span className="absolute bottom-1 right-1 w-2 h-2 bg-[#c3ad6b] rounded-full"></span>
        )}
      </div>
    );
  };
  ...
  return (
    <div
      ref={calendarRef}
      className="p-4 md:p-8 text-center max-w-full overflow-x-auto bg-white font-sans"
    >
              ...
              onChange={async (e) => {
                ...
                try {
                  const resp = await axios.get(
                    `https://ashoka-b.vercel.app/api/bookings/search?q=${encodeURIComponent(
                      val
                    )}`
                  );
